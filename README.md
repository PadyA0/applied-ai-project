...
The interactions above are excerpts. The full untrimmed logs live in [`evidence/`](evidence/), and none of them are pasted from a terminal I happened to have open. They are all generated by one command:

```bash
python capture_evidence.py
```

Every log carries a header with the exact command, the working directory and the exit code, so anything I claim in this README can be traced back to something you can run yourself.
...
The recommender and the harness are deterministic, so 01, 02, 04 and 05 come back byte for byte identical on every run at the same commit. In 03 only the elapsed time on the last pytest line moves. I verified this by running the capture twice and diffing the two folders.

The two failure logs are produced inside temporary directories, so no real data file is ever renamed or deleted to generate them. That matters because a capture script that breaks the repository to prove a point is not one anybody should run.

Detailed per check results are also written to [`tests/results/reliability_report.md`](tests/results/reliability_report.md) every time the harness runs.

### Where things live

| Folder | Contents |
| --- | --- |
| [`evidence/`](evidence/) | Generated command output logs |
| [`diagrams/`](diagrams/) | [`architecture.mmd`](diagrams/architecture.mmd), the mermaid source for the system diagram |
| [`assets/`](assets/) | Supplementary screenshots ([main output](assets/main_output.png), [three profiles](assets/three_profiles.png)) |
| [`tests/results/`](tests/results/) | junit.xml, pytest output, reliability report |
| [`data/`](data/) | The song catalog and the fun fact corpus |

The screenshots in `assets/` are from an earlier version with a 40 song catalog, kept only as a visual record. The logs in `evidence/` are the current, checkable version of the same thing.
